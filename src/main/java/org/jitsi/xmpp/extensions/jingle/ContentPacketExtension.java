 ...
 * being initiated. Among other things it contains details such as transport, or
 * (in the case of RTP) payload type list.
 *
 * @author Emil Ivov
 */
public class ContentPacketExtension extends AbstractPacketExtension {
    ...
    /**
     * Creates a new <tt>ContentPacketExtension</tt>. instance with only
     * required parameters.
     *
     ...
     *
     * @param creator indicates which party originally generated the content
     * type interpreted by the recipient
     * @param name the content type according to the creator
     */
     ...
     * attribute SHOULD be one of the values registered in the IANA Mail Content
     * Disposition Values and Parameters Registry. The default value of this
     * attribute is "session".
     *
     ...
     * <tt>null</tt> if it doesn't. The name argument is a unique identifier for
     * the content type according to the creator, which MAY have meaning to a
     * human user in order to differentiate this content type from other content
     * types (e.g., two content types containing video media could differentiate
     * between "room-pan" and "slides"). If there are two content types with the
     * same value for the 'name' attribute, they shall understood as alternative
     * definitions for the same purpose (e.g., a legacy method and a
     * standards-based method for establishing a voice call), typically to
     * smooth the transition from an older technology to Jingle.
     *
     ...
     * that will be generating content in this session; the allowable values are
     * defined in the <tt>SendersEnum</tt>.
     *
    ...
    /**
     * Sets the value of the senders argument which indicates the parties that
     * will be generating content in this session; the allowable values are
     * defined in the <tt>SendersEnum</tt>.
     *
