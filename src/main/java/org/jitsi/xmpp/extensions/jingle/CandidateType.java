...
public enum CandidateType {
     ...
     * ones, such as ones obtained through Virtual Private Networks (VPNs) and
     * Realm Specific IP.
     */
    host,
    /**
     ...
     * candidate to a TURN server. The relayed candidate is resident on the TURN
     * server, and the TURN server relays packets back towards the agent.
     */
    relay,
    /**
     * Indicates that a candidate is a Server Reflexive Candidate:<br/>
     * A candidate whose IP address and port are a binding allocated by a NAT
     * for an agent when it sent a packet through the NAT to a server. Server
     * reflexive candidates can be learned by STUN servers using the Binding
     * request, or TURN servers, which provides both a relayed and server
     * reflexive candidate.
     */
    srflx,
    /**
