...
public enum JingleAction {
     ...
     * existing content definition(s). Therefore it is the responsibility of the
     * recipient to maintain a local copy of the current content definition(s).
     * If the recipient wishes to include the new content definition in the
     * session, it MUST send a <tt>content-accept</tt> action to the other
     * party; if not, it MUST send a <tt>content-reject</tt>
     ...
     * disposition type is something other than "session" (e.g., "early-session"
     * for early media).
     *
     * In the <tt>session-accept</tt> stanza, the <jingle/> element MUST contain
     * one or more <content/> elements, each of which MUST contain one
     ...
     * method, typically for fallback to a different method (e.g., changing from
     * ICE-UDP to Raw UDP for a datagram transport, or changing from SOCKS5
     * Bytestreams to In-Band Bytestreams [27] for a streaming transport). If
     * the recipient wishes to use the new transport definition, it MUST send a
     * transport-accept action to the other party; if not, it MUST send a
     * transport-reject action to the other party.
     */
     ...
     *
     * @param actionName the name of the <tt>JingleAction</tt> we'd like to
     * create.
     */
     ...
     * or "transport-accept"). The name returned by this method is meant for use
     * directly in the XMPP XML string.
     *
