 ...
 * Start the recording:
 *
 * 1. Send Jibri IQ with {@link Action#START} to Jibri. 2. Jibri replies with
 * RESULT and status {@link Status#PENDING}. 3. Jibri sends SET IQ with status
 * {@link Status#ON} once recording actually starts.
 *
 * Stop the recording:
 *
 * 1. Send Jibri IQ with {@link Action#STOP} to Jibri. 2. Jibri replies with
 * {@link Status#OFF} immediately if the recording has been stopped already or
 * sends separate Jibri SET IQ later on if it takes more time.
 *
 * @author lishunyang
 * @author Pawel Domas
 */
public class JibriIq
     ...
     * used by Jibri participant when it enters Jitsi Meet conference. The value
     * is "displayname".
     */
    ...
    /**
     * The name of the XML attribute which stores whether or not Jicofo should
     * retry this request. Used when the response indicates an error.
     */
     ...
     * {@link #STREAM_ID_ATTR_NAME} is, then it defaults to 'stream'. But if the
     * {@link #STREAM_ID_ATTR_NAME} is not present then it defaults to 'file'.
     * Note that the defaults logic is handled on Jicofo level rather than this
     * packet's extension implementation.
     *
     * In the 'stream' mode Jibri live streams the conference recording. The
     * 'file' mode makes Jibri write the recording to a file.
     */
    ...
    /**
     * An optional description for the 'OFF' state which can be used to describe
     * an 'unclean' to transition to off (e.g. 'error')
     */
    private FailureReason failureReason = null;

    /**
     * In the event of an error, this field denotes whether or not Jicofo should
     * retry the request (for which this error response corresponds) with
     * another Jibri.
     */
    ...
    /**
     * The YouTube broadcast ID for the currently active stream. This is
     * combined with a known URL to generate the URL to view the stream.
     */
    private String youTubeBroadcastId = null;

    /**
     * The ID for this Jibri session. This ID is used to uniquely identify this
     * session (i.e. this particular file recording, live stream or SIP call).
     * It is returned in the ACK of the initial start request and should be used
     * in all subsequent IQ messages regarding this session. When Jibri joins
     * the call, it will use this same session ID in its presence so that an
     * association can be made between this signaling flow and the Jibri client.
     */
    private String sessionId = null;

    /**
     * A JSON-encoded string containing arbitrary Jibri application data. This
     * allows new fields we want to pass to Jibri to be added here so that we
     * don't need to add new explicit fields every time.
     */
     ...
     *
     * @param youTubeBroadcastId a <tt>String</tt> for the stream id attribute
     * or
     * <tt>null</tt> to remove it from XML element.
     */
    ...
    public enum Action {
         ...
         *
         * @return <tt>Action</tt> value for given string or {@link #UNDEFINED}
    ...
    public enum RecordingMode {
         ...
         * {@link #UNDEFINED} if given string does not reflect any of valid
         * values.
         */
